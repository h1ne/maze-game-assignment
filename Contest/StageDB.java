...
class StageDB {
    ...
    private static Stage goalStage = null;
    static private MediaPlayer mainSound = null;
    static private MediaPlayer gameOverSound = null;
    static private Class mainClass;
    static private final String mainSoundFileName = "sound/ダンジョンBGM.mp3"; // BGM by OtoLogic
    static private final String subSoundFileName = "sound/JoyToTheWorld.mp3"; // BGM by OtoLogic
    ...
    public static MediaPlayer getSubSound() {
        if (mainSound == null) {
            try {
                Media s = new Media(new File(subSoundFileName).toURI().toString());
    ...
    public static Stage getGoalStage() {
        if (goalStage == null) {
            try {
                System.out.println("StageDB:getGoalStage()");
                FXMLLoader loader = new FXMLLoader(mainClass.getResource("MapGoal.fxml"));
                VBox root = loader.load();
                Scene scene = new Scene(root);
                goalStage = new Stage();
                goalStage.setScene(scene);
            } catch (IOException ioe) {
                System.err.println(ioe);
            }
        }
